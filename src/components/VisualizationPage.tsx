...
const VisualizationPage: React.FC<VisualizationPageProps> = ({ cacheVisualization }) => {
  ...
  const generateVisualizationWithGemini = async (content: string, type: 'quick' | 'detailed') => {
    ...
    const prompt = `You are a data visualization expert. Create a complete, interactive HTML page with Chart.js that visualizes the following data/information. The visualization should match this EXACT dark theme color scheme:

REQUIRED STYLING:
- Background: Dark theme (#1a1a2e, #16213e gradients)
- Primary colors: #FF4500 (RocketHub orange), #FF6B35, #FFAD5A
- Text: White (#ffffff) and light gray (#e2e8f0)
- Cards/containers: Dark backgrounds with subtle borders
- Professional, modern design matching a corporate dashboard

CONTENT TO VISUALIZE:
${content}

REQUIREMENTS:
1. Create a complete HTML document with Chart.js
2. Use appropriate chart types (line, bar, doughnut, etc.) based on the data
3. Include key metrics as cards/badges
4. Make it responsive and professional
5. Use the exact color scheme specified above
6. Include proper titles and labels
7. Add subtle animations and hover effects
8. Make it look like a premium corporate dashboard
...
Return ONLY the complete HTML code, no explanations.`;
    ...
    try {
      ...
      console.log('✅ Generated HTML length:', generatedText.length);
...
<body>
  ...
  const generateVisualization = async (content: string, type: 'quick' | 'detailed') => {
    try {
      ...
      console.log('🎭 Falling back to mock visualization due to:', error);
